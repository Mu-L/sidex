...
mod tests {
    ...
    fn ctx(dir: &std::path::Path) -> ToolContext {
        ToolContext { cwd: dir.display().to_string() }
    }
    ...
    fn test_read_write_file() {
        ...
        let resp = execute(&req("write_file", r#"{"path":"hello.txt","content":"hello world"}"#), &c);
    ...
    fn test_edit_file() {
        ...
        let resp = execute(&req("edit_file", r#"{"path":"test.txt","old_string":"hello","new_string":"goodbye"}"#), &c);
        assert!(resp.error.is_empty(), "{}", resp.error);
    ...
    fn test_edit_file_not_found() {
        ...
        let resp = execute(&req("edit_file", r#"{"path":"f.txt","old_string":"xyz","new_string":"q"}"#), &c);
    ...
    fn test_shell() {
        let dir = tempfile::tempdir().unwrap();
        let resp = execute(&req("shell", r#"{"command":"echo hello-sidex"}"#), &ctx(dir.path()));
        assert!(resp.error.is_empty(), "{}", resp.error);
    ...
    fn test_shell_bad_dir() {
        let resp = execute(&req("shell", r#"{"command":"echo hi","working_directory":"/nope/fake"}"#), &ToolContext { cwd: "/tmp".into() });
    ...
    fn test_multi_edit() {
        ...
        let resp = execute(&req("multi_edit", r#"{"path":"m.txt","edits":[{"old":"alpha","new":"one"},{"old":"beta","new":"two"}]}"#), &c);
        assert!(resp.error.is_empty(), "{}", resp.error);
    ...
    fn test_batch_read() {
        ...
        let resp = execute(&req("batch_read", r#"{"paths":["x.txt","y.txt"]}"#), &ctx(dir.path()));
        assert!(resp.error.is_empty(), "{}", resp.error);
    ...
    fn test_empty_args() {
        let dir = tempfile::tempdir().unwrap();
        let resp = execute(&req("cwd", ""), &ctx(dir.path()));
        assert!(resp.error.is_empty(), "empty args should be OK: {}", resp.error);
    }

    #[test]
    fn test_unknown_tool() {
        let resp = execute(&req("not_real", "{}"), &ToolContext { cwd: "/tmp".into() });
        assert!(!resp.error.is_empty());
    ...
    fn test_read_file_with_offset_limit() {
        ...
        let resp = execute(&req("read_file", r#"{"path":"lines.txt","offset":2,"limit":2}"#), &ctx(dir.path()));
        assert!(resp.error.is_empty(), "{}", resp.error);
    ...
    fn test_image_read() {
        ...
        let resp = execute(&req("image_read", r#"{"path":"test.png"}"#), &ctx(dir.path()));
        assert!(resp.error.is_empty(), "{}", resp.error);
